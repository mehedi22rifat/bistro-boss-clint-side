...
const Login = () => {
  ...
  const handleValidateCaptcha = (e) =>{
    ...
    return (
          ...
          <div className="w-full flex flex-col mt-11 p-4 max-w-md rounded-md shadow sm:p-8">
	<h2 className="mb-3 text-3xl font-semibold text-center">Login to your account</h2>
	<p className="text-sm text-center">Dont have account?
		<Link to={'/register'} className="focus:underline hover:underline text-red-500">Sign up here</Link>
	</p>
	...
	<form onSubmit={handleLoginForm} noValidate="" action="" className="space-y-8">
		<div className="space-y-4">
            ...
            <div className="space-y-2">
				<div className="flex justify-between">
					<label htmlFor="" className="text-sm"> <LoadCanvasTemplate /></label>
				</div>
                <input onBlur={handleValidateCaptcha} type="capch" name="recapch" id="rechapch" placeholder="Type the chepcha" className="w-full px-3 py-2 border rounded-md dark:border-gray-300 dark:bg-gray-50 dark:text-gray-800 focus:dark:border-violet-600" />
			</div>
		</div>
		<button disabled={disabled} type="submit" className="w-full px-8 py-3 font-semibold rounded-md btn bg-purple-800 text-white">Sign in</button>
	</form>
</div>
</div>    
    );
